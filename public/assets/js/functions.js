export const renderMessages = (messages) => {
  if (messages) {
          ...
          message.replies.length
                      ...
                      ${
                        message.replies.length > 0
                          ...
                          : `<div class="border rounded p-4">
                              <p class="m-0 fw-lighter">There are still no questions. Be the first to ask.</p>
                            </div>`
                      }
                  ...
                  <div>
                    <form action="/consultas/${
                      ...
                      <input class="border-0 bg-light p-2 px-3 w-100" type="text" required name="message" style="border-radius: 1rem;" id="message-input" placeholder="Leave a comment."/>
                      <button class="chat_button bg-danger">Answer</button>
                    </form>
                  </div>
                </div>`;
      })
    ...
    const html = `
      <p>There are still no questions. Be the first to ask.</p>
    `;
    document.getElementById("messageList").innerHTML = html;
  }
};
