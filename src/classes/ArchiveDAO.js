...
class ArchiveDAO {
  ...
  async updateById(array, id, product) {
    try {
      const index = array.findIndex((item) => item.id == id); // Index search for the product array by 'id'
      array = array.filter((item) => item.id != id); // Delete selected product
      product.id = parseInt(id); // Add 'id'
      product.timestamp = Date.now(); // Add 'timestamp'
      array.splice(index, 0, product); // Add product to array
